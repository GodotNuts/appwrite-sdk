...
func __match_resource(type: int, params: Dictionary = {}) -> String:
    ...
    match type:    
		# Client/Server
        AvatarsTask.Task.GET_CREDIT_CARD: resource = _REST_BASE+"/credit-cards/" + params.code + (params.query if params.has("query") else "")
        AvatarsTask.Task.GET_BROWSER_ICON: resource = _REST_BASE+"/browsers/" + params.code + (params.query if params.has("query") else "")
        AvatarsTask.Task.GET_COUNTRY_FLAG: resource = _REST_BASE+"/flags/" + params.code + (params.query if params.has("query") else "")
        AvatarsTask.Task.GET_AVATAR_IMAGE: resource = _REST_BASE+"/image" + (params.query if params.has("query") else "")
		AvatarsTask.Task.GET_FAVICON: resource = _REST_BASE+"/favicon" + (params.query if params.has("query") else "")
		AvatarsTask.Task.GET_QR: resource = _REST_BASE+"/qr" + (params.query if params.has("query") else "")
		AvatarsTask.Task.GET_INITIALS: resource = _REST_BASE+"/initials" + (params.query if params.has("query") else "")
    return resource
